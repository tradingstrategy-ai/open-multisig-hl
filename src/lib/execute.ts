...
function multiSigActionHash(
  ...
  if (vaultAddress) {
    const addrHex = vaultAddress.startsWith("0x")
      ? vaultAddress.slice(2)
      : vaultAddress;
...
async function signMultiSigEnvelope(
  ...
  try {
    ...
    const code =
      ...
      (switchErr as { data?: { originalError?: { code?: number } } })?.data
        ?.originalError?.code;
    if (code === 4902) {
      await provider.request({
      ...
      throw new Error(
        "Please switch your wallet to Arbitrum Sepolia (chain 421614) before executing.",
      );
    }
  }
